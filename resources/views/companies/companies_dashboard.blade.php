		...
		@foreach($feed_content as $content)
			...
			<div class="row">
				<div class="connection_feed">
					<p class="dashboard_category connect">Connection&nbsp;&nbsp;&nbsp;<span class="dashboard_time">{{ $content->created_at->format('m\.j\.y H:ma') }}</span></p>
					<div class="row">
			...
			<div class="row">
				<div class="rfp_feed">
					<p class="dashboard_category rfp">RFP&nbsp;&nbsp;&nbsp;<span class="dashboard_time">{{ $content->created_at->format('m\.j\.y H:ma') }}</span></p>
					<a href="{{ action('RFPController@show', ['id' => $content->id]) }}" alt="{{ $content->project_title }}">
						<h4 class="text-center">{{ $content->project_title }}</h4>
					</a>
					<p class="text-center"><span class="strong">Deadline:</span> {{ $content->deadline->format('F j, Y') }}</p>
					<p class="text-center"><span class="strong">Project Dates:</span> <span class="rfp_date">{{ $content->contract_from_date->format('F j, Y') }} - {{ $content->contract_to_date->format('F j, Y') }}</span></p>
				</div>
			</div>
			@elseif($content->title)
			<div class="row">
				<div class="event_feed">
					<p class="dashboard_category event">Event&nbsp;&nbsp;&nbsp;<span class="dashboard_time">{{ $content->created_at->format('m\.j\.y H:ma')}}</span></p>
					<div class="row">
						<div class="col-xs-3 col-sm-3 col-md-3 col-lg-3 col-xl-3">
